...
{	
	...
	public Customer findCustomerByUsername(String username){
		...
		Query qry = session.createQuery("from Customer c where c.emailId='"+username+"'");
	...
	public List<Customer> findCustomers(String search) {
		Session session= sessionFactory.openSession();
		Query qry = session.createQuery("from Customer c where upper(c.firstName) like'%"+search.toUpperCase()+"%'");
		List<Customer> customers = (List<Customer>)qry.getResultList();
		session.close();			
		return customers;
	}		
